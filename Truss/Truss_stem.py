...
class TrussView():
    ...
    def drawAGrid(self, DeltaX=10, DeltaY=10, Height=320, Width=180, CenterX=120, CenterY=60):
        ...
        """
        """
        This makes a grid for reference.
        """
        ...
        # Draw vertical lines
        for x in range(xStart, xEnd, DeltaX):
            self.scene.addLine(x, yStart, x, yEnd, self.penGridLines)
        # Draw horizontal lines
        for y in range(yStart, yEnd, DeltaY):
            self.scene.addLine(xStart, y, xEnd, y, self.penGridLines)
        #JES MISSING CODE HERE$


    def drawLinks(self, truss=None):
        """ Draw the links of the truss."""
        for link in truss.links:
            node1 = truss.getNode(link.node1_Name)
            node2 = truss.getNode(link.node2_Name)
            if node1 and node2:
                self.scene.addLine(node1.position.x, node1.position.y,
    ...
    def drawNodes(self, truss=None, scene=None):
        """
        Draw the nodes of the truss.
        """
        for node in truss.nodes:
            circle = QGraphicsEllipseItem(node.position.x - 5, node.position.y - 5, 10, 10)
            ...
            label.setPos(node.position.x, node.position.y)
    ...
    def drawALabel(self, x,y,str='', pen=None, brush=None, tip=None):
        """
        Draw a label on the scene.
        """
        label = QGraphicsTextItem(str)
        label.setPos(x, y)
    ...
    def drawACircle(self, centerX, centerY, Radius, angle=0, brush=None, pen=None, name=None, tooltip=None):
        ...
        """
        circle = QGraphicsEllipseItem(centerX - Radius, centerY - Radius, 2 * Radius, 2 * Radius)
        circle.setPen(pen if pen else self.penNode)
        circle.setBrush(brush if brush else self.brushNode)
        if tooltip:
